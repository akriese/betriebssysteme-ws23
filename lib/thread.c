...
thread_control_block *const tcbs =
    (thread_control_block *)_INTERNAL_THREADS_TCB_ARRAY_START;

thread_management *const t_management =
    (thread_management *)_INTERNAL_THREADS_MANAGEMENT_START;

void __create_thread(unsigned int id, int (*fun)(void *), void *input) {
  thread_control_block *new_tcb = &tcbs[id];

  // reset tcb content to get rid off previous content
  memset(new_tcb, 0, sizeof(thread_control_block));

  thread_context *new_ctx = &new_tcb->ctx;
  ...
  t_management->last_created_id = id;
}
 ...
 */
void thread_finish() {
  unsigned int finished_thread_id = t_management->active_thread_id;
...
void thread_save_context(unsigned int thread_id, thread_context *context) {
  memcpy(context, &tcbs[thread_id].ctx, sizeof(thread_context));
}
...
void thread_sleep(unsigned int duration) {
  int thread_id = t_management->active_thread_id;

  // set the thread's status to asleep
  thread_wait(RESOURCE_WAITING_TIME);

  // register thread's sleep end time
  tcbs[thread_id].wake_up_time = t_management->time_counter + duration;
}
